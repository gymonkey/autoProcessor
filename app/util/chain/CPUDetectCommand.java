...
public class CPUDetectCommand implements Command{
	...
	public boolean execute(Context context) throws Exception {
		...
		try{
			pojo.Command cmd = new pojo.Command(fault.hostIP, ServerConnector.CPU_DETECT, latch);
			cmdExecutor.offer(cmd);
			latch.await();
			if(cmd.getResult().equalsIgnoreCase("true")){
				fault.type = FaultType.CPU_FAULT;
