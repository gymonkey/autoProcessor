...
public class JVMDetectCommand implements Command{
	...
	public boolean execute(Context context) throws Exception {
		...
		try{
			pojo.Command cmd = new pojo.Command(fault.hostIP, ServerConnector.JVM_DETECT, latch);
			cmdExecutor.offer(cmd);
			latch.await();
			if(cmd.getResult().equalsIgnoreCase("true")){
				fault.type = FaultType.JVM_FAULT;
