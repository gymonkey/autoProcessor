...
public class JVMProcessCommand implements Command{
	...
	public boolean execute(Context context) throws Exception {
		...
		if(type.equalsIgnoreCase(JobType.JVM_JOB)){
			Job job = (Job)context.get("JOB");
			String hostName = job.hostIP;
			CountDownLatch latch = new CountDownLatch(1);
			pojo.Command cmd = new pojo.Command(hostName, ServerConnector.JVM_PROCESS, latch);
